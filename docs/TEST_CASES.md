...
**Note:** TC-13.1 and TC-13.6 below describe the original form (loan amount/rate/fees typed in fresh each generation) — superseded by FR-13c's financial-model-driven Generate panel (no more numeric form; amounts come from documents or per-line manual entry). Kept for history; see FR-13c's own test cases (TC-13c.1–13c.6) for current behavior.
...
## FR-13c — Deal Financial Model

| ID | Scenario | Role | Steps | Expected |
|---|---|---|---|---|
| TC-13c.1 | Party amounts extracted directly from documents, not even-split | Any Deal Room member | Upload a Lenders-folder document naming two lenders with different real dollar commitments (e.g. 60/40 split) | Each lender's `DealFinancialLine.amount` matches their own stated figure, not `loan_amount / lender_count` (verified live against Riverside Logistics Loan's real sample: Timberline $7,200,000 / Anchor Point $4,800,000 on a $12,000,000 facility, not $6M/$6M each) |
| TC-13c.2 | Missing amount surfaces as a fillable field, not a guess | Deal Team Member | Open "Generate Fund Flow Document" for a deal where a document named a party but stated no dollar figure (e.g. a percentage only) | That line shows an empty input, pre-filled amounts elsewhere are read-only; typing a value and clicking "Save & Generate" writes it as `amount_source: manual_entry` and proceeds |
| TC-13c.3 | Generate refuses an incomplete or unbalanced model | Deal Team Member | Attempt `POST /funding-document/generate` (or click Save & Generate) while a line is still missing, or while Sources ≠ Uses | 400 with a clear message naming what's missing or the exact imbalance (verified live: real extraction from Riverside Logistics Loan's sample docs correctly reported "Sources (12,000,000.00) and Uses (15,290,900.00) don't balance" rather than silently generating) |
| TC-13c.4 | One model powers diagram, PDF, and Remittances identically | Any Deal Room member | Compare the Deal Map diagram, the generated PDF's Sources/Uses, and the Remittances panel for the same deal | All three show the same party names and amounts — none run independent arithmetic (verified live against LFP Batteries Inc: diagram, PDF page 1, and `get_remittance_readiness()` all agree) |
| TC-13c.5 | Uploaded Fund Flow Document replaces the model | Deal Team Member | Upload a settlement/closing PDF and designate it the Fund Flow Document | Every existing `DealFinancialLine` for the deal is replaced (not merged) with lines extracted from the uploaded document's own text, tagged `amount_source: uploaded_fund_flow_document` |
| TC-13c.6 | Remittance readiness no longer requires a generated document | Any Deal Room member | Open Remittances for a deal with real financial-model data but no Fund Flow Document generated yet | Reconciliation shows real per-party rows (amount known + SSI status) instead of empty groups (verified live: Riverside Logistics Loan showed correct Ready/pending rows for borrower, both lenders, and three 3rd-party providers with `deal.fund_flow_document_id` still null) |
| TC-13c.7 | A lender with both a source and fee lines dedupes to one row | Any Deal Room member | View Remittances for a lender who has a principal (source) line and separate fee (use) lines, e.g. Upfront Fee/Interest/Lead Agent Fee | Exactly one row for that lender, `confirmed: true` only if every one of their lines has a known amount (verified live: LFP Batteries Inc's Pacific Crest Capital Partners has 4 financial lines, shows as 1 reconciliation row) |
| TC-13c.8 | Borrower net proceeds is derived, not extracted or typed | Deal Team Member, deal with real Sources and non-borrower Uses fully known | Open Generate Fund Flow Document | The borrower's Use line shows a computed figure equal to `total_sources − sum(non-borrower uses)`, labeled "(derived)", not an editable input, and not the raw dollar figure the borrower's own document happens to state (verified live: LFP Batteries Inc's borrower doc states "Total Facility Amount: USD 8,000,000" but the model correctly shows USD 6,917,000 after the real USD 1,083,000 legal fee; Riverside Logistics Loan similarly derives USD 8,709,100 after USD 3,290,900 in real fees across three 3rd-party providers) |
| TC-13c.9 | Borrower line shows "Pending" while other amounts are still missing | Deal Team Member, deal with at least one non-borrower Use amount still unknown | Open Generate Fund Flow Document | The borrower's Use line shows "Pending — calculated once every other amount is known" instead of an input box or a stale extracted figure |
| TC-13c.10 | Manual entry is refused for a borrower line even via direct API call | Deal Team Member | `POST /financial-model/fill` with a `line_id` belonging to a `role: "borrower"` line | Request succeeds (200) but the line's amount is unchanged — `fill_missing_line()` silently no-ops for borrower-role lines rather than letting a typed figure override the derivation rule |
| TC-13c.11 | Deal Map does NOT re-extract documents — fast, pure read | Any Deal Room member | Click Deal Map for a deal with several documents | Resolves near-instantly (verified live: 88ms for a 5-document deal, down from ~7.4s when this was tried with an eager resync) — no LLM calls happen on this click |
| TC-13c.12 | Generate panel DOES re-sync every current document on open | Deal Team Member | Open "Generate Fund Flow Document" for a deal with several documents | Real extraction re-runs against every current document (verified live: several seconds for a multi-document deal, not an instant cached read) — this is the one place freshness is worth the latency, since it's the editing screen |
| TC-13c.13 | A manually-entered value survives a re-sync that finds nothing | Any Deal Room member | A financial line has `amount_source: "manual_entry"`; the Generate-panel resync runs again and fresh extraction on that same document returns no items | The manual line's amount and `amount_source` are unchanged — verified directly (not just by inspection): flipped a real extracted line to a fake manual value, ran the sync with an empty extraction result, confirmed the manual value survived; then ran it again with real extraction and confirmed it correctly self-healed back |
| TC-13c.14 | Moving a document to a different folder corrects its financial lines on the next Generate-panel open | Deal Team Member | Move a document with existing financial lines from one party folder to another (e.g. Lenders → 3rd Party Providers), then open Generate | That document's lines show the new folder's `role`/`flow`, not the stale one — Deal Map alone will NOT pick this up (TC-13c.11), only an action that triggers a resync (Generate panel open, or Save & Generate) |
| TC-13c.15 | The 3-second background poll never triggers extraction | Any Deal Room member | Sit in a Deal Room for 10+ seconds on a deal with several documents, without opening Generate or Deal Map | No LLM calls happen — `GET /deals/{id}/funding-document` (the poll target) reads `get_deal_financial_model()` directly, which is a pure DB read with no resync call of its own |
| TC-13c.14 | One `ai_api` failure doesn't break the whole page | Any Deal Room member | `ai_api` is unreachable or errors while re-syncing one document among several | That document's existing lines are left as-is; the rest of the deal's documents still sync normally and the page loads without a 500 |
...
| TC-13a.7 | Deal Map opens the visual diagram in the center panel | Any Deal Room member, deal with a generated Fund Flow Document | Click the Deal Map rail icon | Center panel shows the fund-flow SVG diagram (lenders → loan facility → borrower/fees/3rd parties) with correct dollar amounts matching the Fund Flow Document; right-hand census drawer opens at the same time; no mojibake in the "·"/"—" characters (verified live: LFP Batteries Inc, 2 lenders + 4 fee lines + 1 legal line summing to $8,000,000) |
| TC-13a.8 | Diagram close button clears it | Any Deal Room member, diagram open | Click the × in the diagram panel header | Center panel reverts to the normal "select a document to preview" empty state |
| TC-13a.9 | Diagram and document preview are mutually exclusive | Any Deal Room member, diagram open | Click a document in the Documents explorer | Diagram closes and the document preview replaces it; reopening Deal Map does not show a stale diagram from a different deal |
| TC-13a.10 | Long party names wrap instead of overflowing | Any Deal Room member, deal with a lender/3rd-party name longer than ~20 characters | Open the diagram for such a deal | Name wraps onto up to two lines inside its node box (or truncates with an ellipsis) rather than spilling past the box edges |
| TC-13a.11 | Empty state for a deal with no generated Fund Flow Document | Any Deal Room member, deal that has never run `/generate-funding-document` or the Generate panel (or whose Fund Flow Document predates the `fund_flow_*` persistence columns) | Click Deal Map | Center panel shows "No Fund Flow Document has been generated for this deal yet" instead of an empty or broken diagram |
| TC-13a.12 | Each use line renders as its own box | Any Deal Room member, deal with a 3rd party billing multiple fee categories on one document (e.g. Advisory Fees + Due Diligence + Filing + Courier) | Open the diagram | One purple box per fee category, each labeled with its own category and the party name — not summed into a single box per party (verified live: LFP Batteries Inc's Hartwell & Boyd LLP shows as 4 separate boxes, not 1) |
| TC-13a.13 | Diagram renders even when the model doesn't balance | Deal Team Member, deal where every line has a known amount but Sources ≠ Uses | Open the diagram | Renders normally with the real (unbalanced) numbers rather than hiding — `has_data` is gated on `all_known`, not `balanced`, so a genuine discrepancy stays visible instead of being suppressed |
| TC-13a.14 | Connectors are right-angle, not curved | Any Deal Room member | Open the diagram | Lines run horizontal-vertical-horizontal through a shared corridor, not as diagonal/curved bezier paths (verified live against a 13-box deal, previously an unreadable tangle of crossing curves) |
| TC-13a.15 | Every box fully contains its own text, at any deal size | Any Deal Room member, deal with long party names/categories (e.g. "Riverside Logistics Group LLC", "Regulatory Filing Review Fees") | Open the diagram | No text overflows past its box's left or right edge — box width fits its own content first (tries one line, only wraps past a generous ceiling); caught live via screenshot: names were previously missing their first/last few characters because the width estimate under-measured real rendered text |
| TC-13a.16 | Amount labels never cluster regardless of box count | Any Deal Room member, deal with 10+ use lines | Open the diagram | Each dollar-amount label sits next to its own destination box, not at a shared midpoint with the hub — no overlapping/stacked labels near the hub's vertical center, which is what happened before this was fixed |
