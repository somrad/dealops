...
**Diagram: right-angle connectors, one box per line item, sized to fit its own text.** Each Use line (borrower/fee/legal) renders as its own node — a party billing four fee categories on one invoice shows as four boxes, since each is a distinct remittance. Connectors are right-angle (horizontal-vertical-horizontal "elbow") paths through a shared corridor rather than curved beziers, and each dollar-amount label sits next to its own destination box rather than at a shared midpoint — both changes needed once a deal had more than a handful of use lines (10+), where curved fan-out and midpoint labels became an unreadable tangle. Box width is fit to its own name/category text (tries one line first, only wraps past a generous width ceiling, char-width estimates deliberately generous so a box errs wide rather than clipping real text on both edges — a real bug caught from a live screenshot, not just reasoned about).

**Re-synced from documents on explicit user actions, not on every read — and deliberately NOT on Deal Map.** `sync_all_financial_lines_for_deal()` re-reads and re-extracts every current document in a party/economics folder; it's called from the Generate panel's open action and from `generate_funding_document()` (the Save & Generate click), but `get_deal_financial_model()` itself is a pure, fast DB read with no LLM calls. This was tried the other way first — resyncing unconditionally inside `get_deal_financial_model()` — per an earlier version of the founder's own direction, but that made the background 3-second poll (`GET /deals/{id}/funding-document`, which keeps the Remittances summary current) kick off a full multi-document extraction pass every 3 seconds, piling up faster than it could finish and effectively hanging the app on any deal with more than a couple of documents. Reverted, then Deal Map's own resync was tried too and also reverted once felt live: "you are right dont go to LLM for deal map - it makes it slow" (verified: ~7.4s → 88ms). A manually-entered value (`amount_source: "manual_entry"`) is preserved whenever a resync's fresh extraction still finds nothing for that document; a fresh extraction that *does* find content always wins, even over a stale manual entry — this is what makes a corrected document (or a document simply *moved* to a different folder, since resyncing re-reads its *current* folder) self-healing without needing a manual delete-and-resync, for the two actions that do trigger it. A per-document `try/except` around the extraction call means one `ai_api` failure leaves that document's existing lines as-is rather than 500ing the whole page.
