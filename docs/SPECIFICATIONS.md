...
- **New (FR-13):** "Lead lender" / "agent bank" isn't an explicit tracked concept anywhere in this app — Upfront Fees and Lead Agent Fees are attributed to whichever lender is *first* in the deal's Lenders folder, purely by convention (see `_lead_lender()` in `funding_document.py`). Needs a real "who's the agent" designation on a lender the day this matters for an actual multi-lender deal.
- **New (FR-13):** Lender pro-rata shares aren't tracked either — both the Sources contribution and the Interest section split evenly across however many lenders are on file, not by each lender's actual committed percentage.
